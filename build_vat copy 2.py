...
def produce_bvh_texture(data_path, output_path="./output/", clip_length=-1, overlap=0, skip_small_clips = False):
    ...
    for file in files:
        ...
        if clip_length > 0:
            ...
            for n in range(clip_count):
                ...
                save_meta(join(output_path, name + '_world_meta.json'), box, max_vertex_count, anim.frames, anim.fps)
...
def produce_local_bvh_texture(data_path, output_path="./output/", clip_length=-1, overlap=0, skip_small_clips = False):
    ...
    for file in files:
        ...
        if clip_length > 0:
            ...
            print(f"height: {height}, clip_length: {length}, overlap:{overlap}, clip_count:{clip_count}")
            print(f"{clip_count} clips with overlap of {overlap}.")
            # clipping_offset = height % clipping_step
            for n in range(clip_count):
                for y in range(length):
...
if __name__ == '__main__':
    ...
    total_count = produce_local_bvh_texture('bvhData/Test2', './UnityProject/Assets/bvh/local/result_clips', 120, 24)
    print(f"total output: {total_count}")
