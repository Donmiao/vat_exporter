...
class Bvh:
    ...
    def _recursive_apply_frame_local(self, joint, frame_pose, index_offset, p_local, r_local, M_parent, p_parent,
                                     currentFrame):
        if joint.position_animated():
            offset_position, index_offset = self._extract_position(joint, frame_pose, index_offset)
        else:
        ...
        if np.count_nonzero(offset_position) == 3 and np.count_nonzero(joint.offset) == 3:  #####
            print(
                f"WARNING!! Joint have position offset at {joint} ({offset_position},{joint.offset}), file name: {self.filename}")  #####
        rotation = np.rad2deg(mat2euler(M))
        # joint_index = list(self.joints.values()).index(joint)
        ...
        for c in joint.children:
            index_offset = self._recursive_apply_frame_local(c, frame_pose, index_offset, p_local, r_local, M, position,
                                                             currentFrame)
