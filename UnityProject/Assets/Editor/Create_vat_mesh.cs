...
public class Create_vat_mesh
{
    ...
    private static void Create()
    {
        ...
        if (active is Texture2D) {
            ...
            Debug.Log(texturePath.Split(dir+"/")[1].Split("_vertex")[0] + "\n" + dir);
            var texName = texturePath.Split(dir + "/")[1].Split("_vertex")[0];
            Debug.Log(Path.Join(dir, texName + "_meta.json"));
            var metaJson = File.ReadAllText(Path.Join(dir, texName+"_meta.json"));
