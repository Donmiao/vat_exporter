...
def produce_bvh_texture(data_path, output_path="./output/"):
    ...
    for file in files:
        ...
        print(f"Creating world space texture of file name: {anim.filename}, frame counts: {anim.frames}")
        ...
        save_vertices_and_rotations_tiff(output_path, name, vertex_tex, rotation_tex)
...
def produce_local_bvh_texture(data_path, output_path="./output/", clipping_step=-1, overlapping_frame=0, skip_small_clips = False):
    files = [join(data_path, f) for f in listdir(data_path) if isfile(join(data_path, f)) and f.endswith('.bvh')]
    # files.sort(key = lambda x: getmtime(x))
    ...
    for file in files:
        ...
        box = anim.box_range
        print(f"file name: {anim.filename}, frame counts: {anim.frames}")  ## box:{box}
        ...
        width = max_vertex_count
        height = anim.frames
        if (skip_small_clips and clipping_step > height):
            continue
        print(f"===================================\nCreating output texture of {anim.filename} [{width}, {height}]")
        ...
        clipping_offset = 0
        if clipping_step > 0:
            clipping_step = min(clipping_step, height)

            vertex_tex = np.zeros((clipping_step + 1, width, 3))  # vertex_tex = np.zeros((height+1, width, 3))
            rotation_tex = np.zeros((clipping_step, width, 3))
            clip_count = int(height / (clipping_step - overlapping_frame))

            print(f"{clip_count} clips with {overlapping_frame} overlapping frame(s).")
            # clipping_offset = height % clipping_step
            for n in range(clip_count):
                for y in range(clipping_step):
                    t = n * (clipping_step - overlapping_frame) + y
                    if t >= anim.frames:
                        break
                ...
                #####
                for i, node_pair in enumerate(anim.node_and_parent_indices):
                    vertex_tex[clipping_step][i][0] = (float)(
                        node_pair[0] + 1) / 1000.0
                    vertex_tex[clipping_step][i][1] = (float)(
                        node_pair[1] + 1) / 1000.0
                    # print(f"node pair: {vertex_tex[height][i][0]}")
                ...
                save_meta(join(output_path, name + '_local_meta.json'), box, max_vertex_count, anim.frames, anim.fps)
                print(f"save output file of clip {n + 1}/{clip_count} as {name} [{width} , {clipping_step}].")
...
if __name__ == '__main__':
    ...
    produce_local_bvh_texture('bvhData/Test', './UnityProject/Assets/bvh/local', 41, 1, True)
